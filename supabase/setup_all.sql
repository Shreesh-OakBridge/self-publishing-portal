...
-- ============================================================
-- Invoice fields (author_name, invoice_number) + numbering trigger
-- (from 20260626120000_invoice_fields.sql)
-- ============================================================
...
DECLARE
  ...
  fy_start int; fy_end int; n bigint;
BEGIN
  IF NEW.invoice_number IS NULL THEN
    IF m >= 4 THEN fy_start := y; fy_end := y + 1;
    ELSE fy_start := y - 1; fy_end := y; END IF;
...
-- ============================================================
-- Project Workspace: messages + proofs (from 20260626140000_project_workspace.sql)
-- ============================================================
...
CREATE POLICY "Send messages on own orders" ON project_messages FOR INSERT TO authenticated
  WITH CHECK (sender_id = auth.uid() AND (is_admin() OR EXISTS (SELECT 1 FROM orders o WHERE o.id = order_id AND o.user_id = auth.uid())));
...
CREATE POLICY "Admins create proofs" ON project_proofs FOR INSERT TO authenticated WITH CHECK (is_admin());
