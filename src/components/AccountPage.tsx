...
import { BookOpen, LogOut, Palette, Calculator, Home, Pencil, ShoppingBag, LayoutDashboard, TrendingUp, IndianRupee } from 'lucide-react';
...
const bookStatus = (s: string) => BOOK_STATUS[s] ?? { label: s.replace('_', ' '), color: 'bg-gray-100 text-gray-600' };
...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoadingData(true);
      const [c, r, o, b] = await Promise.all([
        supabase
      ...
      if (b.data) setBooks(b.data as AuthorBook[]);
